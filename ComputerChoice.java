...
public class ComputerChoice {
    int team;
    private boolean CPU_won = false;
    private boolean human_won = false;
    ...
    public int makeSelection(int[][] boardChoice, int team) {
        ...
        col_and_score = mini_max(boardChoice, depth, true, AI_team, -1);
    ...
    //minimax
    public ArrayList mini_max(int [][] board, int depth, boolean maximizing_team, int AI_team, int prev_move) {
        ...
        boolean humanWon = checkBoard.checkWon(board, -AI_team);
        boolean filled = checkBoard.checkFilled(board);

         //base case, checks for terminal nodes 
         if (depth == 0 || AI_won || humanWon|| filled) {
            //check terminal states
            if ( AI_won || humanWon|| filled) {
                if (AI_won == true) {
                    col_and_score.add(prev_move);
                    col_and_score.add(100000);
        ...
        if (maximizing_team) {
            ...
            for (int i = 0; i < valid_moves.size(); i++) {
                ...
                ArrayList<Object> score = mini_max(board_copy, depth-1, false, AI_team, i);
            ...
            for (int i = 0; i < valid_moves.size(); i++) {
                ...
                State newState = new State(board_copy);
                newState.place(board_copy, valid_moves.get(i), -AI_team);
                ArrayList<Object> score = mini_max(board_copy, depth-1, true, AI_team, i);
                minmax_score =  (int) score.get(1);
                if (minmax_score < value) {
                    value = minmax_score;
                    best_col = valid_moves.get(i);
